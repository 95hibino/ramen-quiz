    ...
    <div className="space-y-6 text-sm leading-relaxed text-ramen-soy/90">
      ...
      <section className="space-y-2">
        ...
        <ul className="ml-5 list-disc space-y-1">
          <li>
            パスワードは認証基盤（Supabase Auth）がハッシュ化して保管します。
            運営者が平文のパスワードを保持・閲覧することはありません。
          </li>
          <li>
            アカウント情報はサーバー上で管理されるため、
            別の端末・ブラウザからでも同じユーザー名とパスワードでログインできます。
          </li>
          <li>
            ユーザー名は 3〜20 文字で、英数字・ひらがな・カタカナ・漢字と
            <code className="rounded bg-ramen-broth/20 px-1">_</code>
            <code className="rounded bg-ramen-broth/20 px-1">-</code> が使用できます。
            他の利用者と同じユーザー名は登録できません（大文字小文字・全角半角は区別しません）。
            登録後のユーザー名の変更はできません。
          </li>
          <li>
            ユーザー名・都道府県・好きなラーメン店は、ランキング画面で他の利用者に公開されます。
          </li>
          <li>
            メールアドレスを登録しない仕様のため、メールによるパスワード再設定は提供していません。
            代わりに、登録完了時に<span className="font-bold">復旧コード</span>を一度だけ表示します。
            復旧コードはその場でしか表示されないため、必ず安全な場所に控えてください
            （ログイン中であれば、マイページからいつでも再発行できます）。
          </li>
          <li>
            <span className="font-bold">
              パスワードと復旧コードの両方を失った場合、アカウントを復旧する手段はありません
            </span>
            。また、ユーザー名は一意のため、同じユーザー名で別のアカウントを作り直すこともできません。
          </li>
        </ul>
      </section>
      ...
      <section className="space-y-2">
        ...
        <ul className="ml-5 list-disc space-y-1">
          ...
          <li>運営者・管理者になりすます行為、またはそのおそれのある名称の使用</li>
